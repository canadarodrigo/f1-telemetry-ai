...
t = {
    ...
    # Novas traduções para o motor de anomalias
    "titulo_anomalia": "### 🔍 Motor de Detecção de Anomalias (Erro Residual)" if is_pt else "### 🔍 Anomaly Detection Engine (Residual Error)",
    "vel_sensor": "Velocidade Sensor (Real)" if is_pt else "Sensor Speed (Actual)",
    "prev_lstm": "Previsão LSTM (Esperado)" if is_pt else "LSTM Prediction (Expected)",
    "erro_residual": "Erro Residual (Desvio)" if is_pt else "Residual Error (Deviation)",
    "msg_normal": "✅ **Telemetria Normal:** Comportamento do carro alinhado com o padrão da volta ideal (Baseline)." if is_pt else "✅ **Normal Telemetry:** Car behavior aligns with the ideal lap pattern (Baseline).",
    "msg_aviso": "⚠️ **Aviso de Degradação:** Desvio estatístico de {0:.2f} km/h detectado. Monitore o desgaste dos pneus." if is_pt else "⚠️ **Degradation Warning:** Statistical deviation of {0:.2f} km/h detected. Monitor tire wear.",
    "msg_critico": "🚨 **ANOMALIA CRÍTICA:** Desvio massivo de {0:.2f} km/h! Possível perda de tração ou anomalia severa de frenagem detectada no sistema." if is_pt else "🚨 **CRITICAL ANOMALY:** Massive deviation of {0:.2f} km/h! Possible loss of traction or severe braking anomaly detected in the system.",
}
...
col3.metric(t["prev_futuro"], f"{velocidade_prev:.2f} km/h", delta=round(velocidade_prev - vel_atual, 2))


# --- NOVA CAMADA: MOTOR DE ANOMALIAS ---
if sucesso_api:
    st.markdown("---")
    st.markdown(t["titulo_anomalia"])
    ...
    ca1.metric(label=t["vel_sensor"], value=f"{vel_atual:.1f} km/h")
    ca2.metric(label=t["prev_lstm"], value=f"{velocidade_prev:.2f} km/h")
    ca3.metric(
        label=t["erro_residual"], 
    ...
    if erro_residual <= LIMIAR_ALERTA:
        st.success(t["msg_normal"])
    elif erro_residual <= LIMIAR_CRITICO:
        st.warning(t["msg_aviso"].format(erro_residual))
    else:
        st.error(t["msg_critico"].format(erro_residual))
